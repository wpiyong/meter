...
namespace gUV.ViewModel
{
    ...
    public class LogEntryViewModel : ViewModelBase
    {
        ...
        object _logEntriesLock = new object();
        ...
        public LogEntryViewModel()
        {
            ...
            System.Windows.Data.BindingOperations.EnableCollectionSynchronization(_logEntries, _logEntriesLock);
        ...
        void ExportLog()
        {
            try
            {
                ...
                NotepadHelper.ShowMessage(buffer, "gColor Log");
            }
            catch (Exception /*ex*/)
            {
            }
        ...
        {
            //foreach (var itemToRemove in LogEntries.Where(x => Convert.ToDateTime(x.DateTime) 
            //    < DateTime.Now.AddHours(-1)).ToList())
            //{
            //    LogEntries.Remove(itemToRemove);
            //}

            if (!GlobalVariables.InTestMode)
                LogEntries.Add(new Model.LogEntry(DateTime.Now, message));
            else
                LogEntries.Add(new Model.LogEntry(DateTime.Now, "TEST MODE: " + message));
#if !DEBUG
            if (showPopup)
            {
                Message = message;
